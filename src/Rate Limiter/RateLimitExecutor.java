...
/**
 *
 * @author aayush This class calculates the TPS and applies the throttling
 *         policy It uses granular write locking for thread safety.
 */
...
// Get the delta time elapsed
		double deltaTime = (currentTime - (long) timeStamp);
...
// What is higher, TPS threshold or transactions per second? Exclude the very first transaction to avoid false positives
		if (tps >= threshold && transactions != 1) {
			...
			RateLimitManager.instance.getThreadPool().execute(new WorkerThread(listener));
		}
// Leave write lock
		wLock.unlock();
	}
